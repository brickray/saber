...
public:
	enum{
		...
		NOP      = 18, //0个参数，占位指令
		ENUM0    = 19, //0个参数指令数量
		GTFILED  = 19, //1个参数，操作栈sp-1, sp-2
		MOVE     = 20, //1个参数，将栈顶的值赋值给参数
		JZ       = 21, //1个参数，比较栈顶的值是否为0, 跳转到参数所指定的地址
		JUMP     = 22, //1个参数，跳转到参数所指定的地址
		CALL     = 23, //1个参数，栈顶值为函数地址，参数为函数参数个数
		RET      = 24, //1个参数，返回地址在栈顶
		RESERVE  = 25, //1个参数，栈中保留n个空位供局部变量使用
		PUSHB    = 26, //1个参数，将布尔值压入栈顶
		PUSHI    = 27, //1个参数，将整数压入栈顶
		PUSHF    = 28, //1个参数，将浮点数压入栈顶
		PUSHS    = 29, //1个参数，将字符串压入栈顶
		ENUM1    = 30, //1个参数指令数量
		PUSH     = 30, //2个参数，将值压入栈顶
		ENUM2    = 31, //2个参数指令数量
		ECNUM    = 31,
